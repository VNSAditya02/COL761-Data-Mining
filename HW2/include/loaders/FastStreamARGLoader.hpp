...
class FastStreamARGLoader: public ARGLoader<Node, Edge> {
    public:
        ...
        FastStreamARGLoader(vector<nodeID_t> t_nodes, vector<pair<pair<nodeID_t, nodeID_t>, Edge>> t_edges, bool undirected=false);
...
template <typename Node, typename Edge>
FastStreamARGLoader<Node,Edge>
::FastStreamARGLoader(vector<nodeID_t> t_nodes, vector<pair<pair<nodeID_t, nodeID_t>, Edge>> t_edges, bool undirected) {
    last_edge_node = NULL_NODE;
    node_count = t_nodes.size();
    nodes.resize(node_count);
    edges.resize(node_count);
    uint32_t edge_count;
    nodeID_t i;
    ...
    for(i=0; i<t_edges.size(); i++){
        // std::cout << t_edges[i].first.first << " " << t_edges[i].first.second << endl;
        edges[t_edges[i].first.first][t_edges[i].first.second] = t_edges[i].second;
        if (undirected){
            edges[t_edges[i].first.second][t_edges[i].first.first] = edges[t_edges[i].first.first][t_edges[i].first.second];
        }    
    }
    // for(i=0; i<node_count; i++) {
    //     in >> edge_count;
